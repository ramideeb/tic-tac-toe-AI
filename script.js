...
function MousePosition(mX, mY, click) {
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      ...
      if (
        squares[i + 3 * j] === null &&
        mX > left && mX < left + cellSize && mY > top && mY < top + cellSize
      ) {
        index = i + 3 * j;
      }
    }
...
function ev(boarde) {
  ...
  ////////////////////////////////////////////////////////
  if (
    (boarde[0] === boarde[4] && boarde[8] === null) ||
...
function AlphaBeta(levels, board, a, b, player, max) {
  var i,
    outcome = isEnded(board),
    childBoard;
  if (outcome.winner !== null) {
    if (outcome.winner === player) {
      return 1000;
    } else if (outcome.winner === 1 - player) {
      return -100;
    } else {
      return 0;
    }
...
function DrawAll() {
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
      DrawCell(
        squares[i + 3 * j],
        boardleft + i * celloffset,
        boardtop + j * celloffset,
        cellSize,
        i + 3 * j === mouseIN
      );
    }
  }
  if (gameStatus.winner === 0 || gameStatus.winner === 1) {
    DrawLine();
  }
}
...
window.addEventListener(
  ...
  e => {
    if (start == 1) {
      if (gameStatus.winner !== null) {
        Init();
      } else if ((currentPlayer === 0 && twoPlayers == 0) || twoPlayers == 1) {
